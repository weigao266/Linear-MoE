...
from linear_moe.data.llava import conversation as conversation_lib
from linear_moe.data.llava.mm_utils import tokenizer_image_token
from linear_moe.data.llava.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
from linear_moe.model.llava.clip_encoder import CLIPVisionTower
...
class LazySupervisedDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in self.list_data_dict[i]:
            data_dict['image'] = image.to(dtype=torch.half if self.args.fp16 else torch.bfloat16)
