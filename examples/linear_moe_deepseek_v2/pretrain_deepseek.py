...
def model_provider(pre_process=True, post_process=True) -> Union[GPTModel, MambaModel, megatron.legacy.model.GPTModel]:
    ...
    if args.use_la_module:
        if args.la_module == "pure_mamba2":
            mamba_stack_spec = get_pure_mamba2_stack_linear_moe_layer_local_spec(args.num_experts, args.moe_grouped_gemm, qk_layernorm=True)
        elif args.la_module == "hybrid_mamba2":
            ...
            transformer_layer_spec = get_gla_linear_moe_layer_local_spec(args.num_experts, args.moe_grouped_gemm, qk_layernorm=True)
        elif args.la_module == "deltanet":
            transformer_layer_spec = get_deltanet_linear_moe_layer_local_spec(args.num_experts, args.moe_grouped_gemm, qk_layernorm=True)
        elif args.la_module == "rwkv6":
            transformer_layer_spec = get_rwkv6_linear_moe_layer_local_spec(args.num_experts, args.moe_grouped_gemm, qk_layernorm=True)
        elif args.la_module == "hgrn2":
            transformer_layer_spec = get_hgrn2_linear_moe_layer_local_spec(args.num_experts, args.moe_grouped_gemm, qk_layernorm=True)
    else:
        if use_te:
